...
public class AICqc : MonoBehaviour
{
        ...
        if (commands.Count == 0)
        {
            ...
            else
            {
                if (CanReachAnyCharacter())
                {
                    ...
                    {
                        newPos = AdvanceToClosestReachableCharacter();
                        if (newPos.x != transform.position.x || newPos.y != transform.position.y)
                        {
                            Grid.GetXY(newPos, out int newX, out int newY);
                            commands.Add(new AICommand(AICommand.Commands.move, newX, newY));
    ...
    private bool CanReachAnyCharacter()
    {
    ...
    private Vector2 AdvanceToClosestReachableCharacter()
    {
        Pathfinding pathfinding = Pathfinding.Instance;
        CharacterUnit character = GetClosestReachableCharacter();
        if(character != null)
        {
            ...
            {
                return shortestPath[Mathf.Min(
                    shortestPath.Count - 1, stats.GetCurrentAP() / stats.GetMovementCost())];
            }
        }
        return transform.position;
    }

    private CharacterUnit GetClosestReachableCharacter()
    {
        List<CharacterUnit> playerCharacters = GetPlayerCharacters();
        Grid.GetXY(transform.position, out int unitX, out int unitY);
        ...
        foreach (CharacterUnit character in playerCharacters)
        {
            if (CharacterIsReachable(character))
            {
    ...
    private bool CharacterIsReachable(CharacterUnit character)
    {
        Pathfinding pathfinding = Pathfinding.Instance;
        GridCell cell;
        Vector2 pos = Vector2.zero;
    ...
    private bool CheckTurnDone()
    {
        ...
        {
            if (stats.GetCurrentAP() >= stats.GetAttackCost()) return false;
            else
            {
